...
export default function DailyReports() {
  ...
  const handleExportReport = () => {
    ...
    try {
      ...
      doc.text('RGBUSS - Daily Report', pageWidth / 2, 20, { align: 'center' });
      
      doc.setFontSize(14);
      doc.setFont('helvetica', 'normal');
      doc.text(`Date: ${format(new Date(selectedDate), 'EEEE, MMMM do, yyyy')}`, pageWidth / 2, 30, { align: 'center' });
      doc.text(`Generated by: ${user?.full_name}`, pageWidth / 2, 40, { align: 'center' });
      doc.text(`Generated at: ${format(new Date(), 'MMM dd, yyyy HH:mm')}`, pageWidth / 2, 50, { align: 'center' });
      ...
      const summaryLines = [
        ...
        `Total Entres: ${totals.totalEntres}`,
        `Total Sortie: ${totals.totalSortie}`,
        `Total Revenue (P.Total): $${totals.totalPTotal.toFixed(2)}`,
        `Credits Found: ${creditsData.length}`,
        `Adjustments Found: ${adjustmentsFound}`,
        isFirstDay ? 'Note: This is the first day - stock values are 0' : 'Stock values based on previous day\'s solde'
      ];
      ...
      if (reportData.length > 0) {
        doc.setFontSize(12);
        doc.setFont('helvetica', 'bold');
        doc.text('1. Daily Report Table', 20, yPosition);
        yPosition += 10;
      ...
      if (creditsData.length > 0) {
        ...
        doc.setFontSize(12);
        doc.setFont('helvetica', 'bold');
        doc.text('2. (DETTE NOM ET PRENOM + MONTANT)', 20, yPosition);
        yPosition += 10;

        const creditTableData = creditsData.map((credit, index) => [
          (index + 1).toString(),
      ...
      doc.text('Generated by RGBUSS Business Management System', pageWidth / 2, footerY, { align: 'center' });
